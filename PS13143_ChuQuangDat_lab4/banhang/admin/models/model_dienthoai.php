...
class model_dienthoai extends model_system{
    ...
    function listDienThoai(){
        $sql = "SELECT * FROM dienthoai ORDER BY idNSX DESC";
    ...
    function update($id, $ten_dt,$mota,$hinh,$hot,$tonkho,$giakm,$idNSX,$gia,$thutu,$an, $ngay){
         $sql = "UPDATE `dienthoai` SET `TenDT` = '$ten_dt', `Gia` = '$gia', `GiaKM` = '$giakm', `UrlHinh` = '$hinh', `ThoiDiemNhap` = '$ngay', `MoTa` = '$mota', `Hot` = '$hot', `idNSX` = '$idNSX', `AnHien` = '$an', `SoLuongTonKho` = '$tonkho', `ThuTu` = '$thutu' WHERE `dienthoai`.`idDT` = $id"; 
        //  if($hinh != '') 
         // else  $sql = "UPDATE `dienthoai` SET `TenDT` = '$ten_dt', 'MoTa' = '$mota', 'UrlHinh' =  '$hinh' , 'idNSX' = '$idNSX', 'TonKho' ='$tonkho','GiaKM' = '$giakm','Gia' = '$gia','ThoiDiemNhap' = '$ngay'          , `ThuTu` = '$thutu', `AnHien` = '$an' WHERE `dienthoai`.`idDT` = $id"; 
